...
export async function PATCH(req: NextRequest) {
  ...
  const backendRes = await backendFetch("/api/users/me/history-max-messages", {
    method: "PATCH",
  ...
  if (!backendRes.ok) {
    // Pass the real status through (400 for out-of-range values, 401 for auth) so the client
